    ...
    public static void main(String[] args) {
        if (args.length != 2) {
            ...
            try {
                ...
                DatagramPacket joinDatagram = new DatagramPacket(joinBuffer, joinBuffer.length, group, port); 
                ...
                while (true) {
                    ...
                    if (message.equalsIgnoreCase(GroupChat.TERMINATE)) {
                        ...
                        DatagramPacket leaveDatagram = new DatagramPacket(leaveBuffer, leaveBuffer.length, group, port); 
                        socket.send(leaveDatagram);
                        socket.leaveGroup(group); 
                        socket.close(); 
                        break; 
                    }
                    message = name + " [" + new Date() + "]: " + message; 
